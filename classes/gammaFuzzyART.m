%% """ Gamma-normalized Fuzzy ART """
% 
% PROGRAM DESCRIPTION
% This is a MATLAB implementation of the "Gamma-normalized Fuzzy ART" network.
%
% REFERENCES
...
% Gamma-normalized Fuzzy ART Class
classdef gammaFuzzyART    
    ...
    properties (Access = private)
        W_old = [];                 % old weight vectors (used for stopping criterion)    
    end
    methods        
        %% Assign property values from within the class constructor
        function obj = gammaFuzzyART(settings) 
        ...
        %% Train
        function obj = train(obj, x, maxEpochs, nSamples) 
            ...
            if isempty(obj.W)             
                ...
                obj.dim = obj.dim/2;    % Assumes the input is already complement coded             
            ...
            while(true)
                ...
                for i=sample_no1:nSamples  % loop over samples
                    ...
                    if mismatch_flag  % If there was no resonance at all then create new category
                        ...
                        obj.n(obj.nCategories, 1) = 1;          % increment number of samples associated with new category
